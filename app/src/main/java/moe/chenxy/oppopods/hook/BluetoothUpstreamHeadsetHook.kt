...
class BluetoothUpstreamHeadsetHook : HookContext() {
    private val TAG = "HybridPods-Upstream"
            ...
            override fun onReceive(context: Context?, intent: Intent?) {
                when (intent?.action) {
                    ...
                    OppoPodsAction.ACTION_PODS_CONNECTED -> {
                        ...
                        currentAddress?.let { knownDeviceAddresses.add(it.uppercase()) }
                    }
    ...
    private fun hookAddressStringResult(binderClass: Class<*>, methodNames: List<String>, label: String, forced: () -> String) {
        ...
        runCatching {
            hookBefore(binderClass.method(methodName, String::class.java)) {
                ...
                if (address == null || !isKnownDeviceAddress(address)) return@hookBefore
                result = forced()
    ...
    private fun handleAddressString(method: String, data: Parcel, reply: Parcel, forced: String): Boolean? {
        ...
        val isOppo = address != null && isKnownDeviceAddress(address)
    ...
    private fun isSupportedPod(device: BluetoothDevice?): Boolean {
        ...
        val result = DeviceType.isSupported(name) || (address != null && isKnownDeviceAddress(address))
        if (result && address != null) knownDeviceAddresses.add(address.uppercase())
        return result
    }
    ...
    private fun realRefreshPayload(): String {
        ...
        if (!hasTransparencyVocalEnhancementState && oppoSnapshot != null) {
            currentTransparencyVocalEnhancement = oppoSnapshot.transparencyVocalEnhancement
            hasTransparencyVocalEnhancementState = true
        }
        val transparencyVocalEnhancement = if (hasTransparencyVocalEnhancementState) {
            currentTransparencyVocalEnhancement
        } else {
            oppoSnapshot?.transparencyVocalEnhancement ?: currentTransparencyVocalEnhancement
        }
        val address = oppoSnapshot?.address ?: miShuaiSnapshot?.address
        address?.let {
            currentAddress = it
            knownDeviceAddresses.add(it.uppercase())
        }
        val deviceName = oppoSnapshot?.deviceName ?: miShuaiSnapshot?.deviceName
        deviceName?.let { currentName = it }
    ...
    private fun isKnownDeviceAddress(address: String): Boolean {
        return address.uppercase() in knownDeviceAddresses
    }
