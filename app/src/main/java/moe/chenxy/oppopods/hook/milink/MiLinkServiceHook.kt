...
object MiLinkServiceHook : HookContext() {
    ...
    internal fun hookStringAddressResult(className: String, methodName: String, result: () -> Any) {
        runCatching {
            hookAfter(findMethod(className, methodName, String::class.java)) {
                ...
                if (!isKnownDeviceAddress(address)) return@hookAfter
                this.result = result()
            }
    ...
    internal fun isSupportedPod(device: BluetoothDevice): Boolean {
        val address = runCatching { device.address }.getOrNull()
        if (address != null && isKnownDeviceAddress(address)) return true
        val name = runCatching { device.name ?: device.alias }.getOrNull().orEmpty()
        val result = DeviceType.isSupported(name)
        if (result && address != null) {
            knownDeviceAddresses.add(address.uppercase())
    ...
    internal fun isKnownDeviceAddress(address: String): Boolean {
        val normalized = address.uppercase()
        return normalized == currentAddress?.uppercase() || normalized in knownDeviceAddresses
    }
    ...
    private fun backendDeviceName(): String? {
        return runCatching { RfcommController.currentStatusSnapshot().deviceName }
            ...
            ?: runCatching { MiShuaiRfcommController.currentStatusSnapshot().deviceName }
                .getOrNull()
                ?.takeIf { it.isNotBlank() }
    }
    ...
    internal fun isTargetAncBatteryModel(model: Any?): Boolean {
        ...
        return device?.let { isSupportedPod(it) } == true
    }
